...
class PSF(Model):
    ...
    def get_model(self, *parameter, offset=None):
        ...
        offset: 2D tuple or ``~spaxlet.Parameter`
...
class FunctionPSF(PSF):
    ...
    Parameters
    ----------
    parameters: `~spaxlet.Parameter` or list thereof
...
class GaussianPSF(FunctionPSF):
    ...
    Parameters
    ----------
    sigma: float, array, or `~spaxlet.Parameter`
...
class DeltaPSF(ImagePSF):
    ...
    Use this in a model :class:`~spaxlet.frame.Frame` when component
