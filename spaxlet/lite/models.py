# This module contains classes and methods to use the so-called "lite" version of spaxlet.
# In this instance "lite" just means spaxlet stripped down to the bare essentials needed
...
# to run faster and use less memory than the standard spaxlet modules while obtaining
# a similar log likelihood.
import numpy as np

from spaxlet.lite.measure import weight_sources
...
class LiteComponent:
    """A base component in spaxlet lite
...
class LiteFactorizedComponent(LiteComponent):
    ...
    def __init__(self, sed, morph, center, bbox, model_bbox, bg_rms, bg_thresh=0.25, floor=1e-20,
        ...
        model_bbox: `~spaxlet.bbox.Box`
            The `Box` that contains the model.
            This is simplified from the main spaxlet, where the model exists
            in a `frame`, which primarily exists because not all
            observations in main spaxlet will use the same set of bands.
        bg_rms: `numpy.array`
...
class LiteSource:
    ...
    def get_model(self, bbox=None, use_flux=False):
        ...
        if use_flux:
            ...
            # (calculated by spaxlet.lite.measure.weight_sources)
            if bbox is None:
                return self.flux
...
class LiteObservation:
    ...
    `Renderer` class from base spaxlet, greatly simplified due
...
class LiteBlend:
    ...
    classes in main spaxlet, greatly simplified due to the assumptions that the
    ...
    def __init__(self, sources, observation):
        ...
        Parameters
        ----------
        sources: `list` of `spaxlet.lite.LiteSource`
            The sources to fit.
        `observation`: `spaxlet.lite.LiteObservation`
