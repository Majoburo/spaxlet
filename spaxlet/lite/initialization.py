...
logger = logging.getLogger("spaxlet.lite.initialization")
...
def init_main_parameters(detect, center, observation,
    ...
    """Initialize parameters using the same general algorithm as spaxlet main
...
def init_adaprox_component(center, bbox, sed, morph, observation, factor=10, bg_thresh=None,
    ...
    Returns
    -------
    component: `spaxlet.lite.LiteFactorizedComponent`
...
def init_all_sources_main(observation, centers, detect=None,
    ...
    algorithms in spaxlet main to give nearly identical resulting sed and
    morphology arrays without creating all of the intermediate spaxlet
    objects.
...
def init_wavelet_source(center, nbr_components, init):
    ...
    Returns
    -------
    source: `spaxlet.lite.LiteSource`
    """
...
def parameterize_sources(sources, observation, parameterization):
    """Convert the parameters in a list of sources in spaxlet lite parameters
