...
logger = logging.getLogger("spaxlet.frame")
...
class Frame:
    ...
    wcs: TBD
        World Coordinates
    psf: `spaxlet.PSF` or its arguments
        PSF in each channel
    ...
    @property
    def bbox(self):
        """The `~spaxlet.bbox.Box` of this `Frame`.
        """
        return self._bbox
    ...
    def from_observations(
        ...
        Parameters
        ----------
        observations: array of `spaxlet.Observation` objects
            array that contains Observations to match onto a common frame
        model_psf: `spaxlet.PSF`
