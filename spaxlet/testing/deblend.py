...
def deblend(data: Dict[str, np.ndarray], max_iter: int, e_rel: float):
    ...
    psf = spaxlet.ImagePSF(data["psfs"])
    filters = settings.filters

    # Initialize the model, frame, observation, and sources
    t0 = time.time()
    model_psf = spaxlet.GaussianPSF(sigma=(0.8,) * len(filters))

    model_frame = spaxlet.Frame(images.shape, psf=model_psf, channels=filters)
    ...
    for k in skipped:
        sources.insert(k, spaxlet.NullSource(model_frame))
