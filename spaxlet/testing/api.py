...
def get_blend(blend_id: str, path: str = None):
    ...
    if path is None:
        ...
        result = client.get_object(Bucket="spaxlet-blends", Key=blend_id)
...
def save_residual(residual_img, blend_id: str, branch: str):
    ...
    aws.upload_file(fp.name, "spaxlet-residuals", get_object_name(branch, blend_id))
...
def deblend_and_measure(
    ...
    :param set_id: ID of the set to analyze.
        ...
        `save` is `True`.
    :param branch: The spaxlet branch to test
    ...
    for bidx, blend_id in enumerate(blend_ids):
        ...
        if plot_residuals or save_residuals:
            import spaxlet.display as display
