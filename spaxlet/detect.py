...
logger = logging.getLogger("spaxlet.detect")
...
def footprint_intersect(footprint1, box1, footprint2, box2):
    ...
    footprint1, footprint2: `spaxlet.detect_pybind11.Footprint`
...
def draw_footprint_box(footprint, ax):
    """Draw a spaxlet Footprint in a plot

    Parameters
    ----------
    footprint: `spaxlet.detect_pybind11.Footprint`
...
class QuadTreeRegion:
    ...
    def add_footprints(self, footprints):
        """Add bounding boxes for a list of spaxlet footprints.

        Parameters
        ----------
        footprints: `list` of `spaxlet.detect_pybind11.Footprint`
            A list of footprints detected by spaxlet.
        """
    ...
    def query(self, other_box=None):
        ...
        Returns
        -------
        result: `set` of `spaxlet.bbox.BoundingBox`
...
class SingleScaleStructure:
    ...
    peaks: `dict`: {`int`, `list` of `spaxlet.detect_pybind11.Peak`}
...
def get_blend_trees(detect):
    ...
    """
    from spaxlet.detect_pybind11 import get_footprints
    all_footprints = []
