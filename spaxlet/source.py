...
logger = logging.getLogger("spaxlet.source")
...
class PointSource(FactorizedComponent):
    ...
    def __init__(self, model_frame, sky_coord, observations):
        ...
        observations: instance or list of `~spaxlet.Observation`
...
class CompactExtendedSource(FactorizedComponent):
    def __init__(
        ...
        sky_coord: tuple
            Center of the source
        observations: instance or list of `~spaxlet.observation.Observation`
            Observation(s) to initialize this source.
        shifting: `bool`
...
class StarletSource(FactorizedComponent):
    ...
    Sources are initialized as `~spaxlet.ExtendedSource`, and the morphology is then
    ...
    def __init__(
        ...
        thresh: `float`
            Multiple of the backround RMS used as a
            flux cutoff for morphology initialization.
        spectrum: `numpy.ndarray` or `spaxlet.Parameter`
...
class MultiExtendedSource(CombinedComponent):
    ...
    Uses `~spaxlet.ExtendedSource` to define the overall morphology,
