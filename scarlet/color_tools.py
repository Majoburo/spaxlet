...
eye_response = StringIO("""380,2.689900e-003,2.000000e-004,1.226000e-002
...
def wavelength_to_rgb(wl, gamma=0.8, output='float'):
    ...
    in angstroms, and will be nonzero in the range from 3800 through 7500
...
def blackbody_spectrum(wavelengths, T=5800):
    '''
    Give the blackbody spectral irradiance corresponding to a given temperature.
    Input wavelengths in angstroms, T in Kelvin, SED returned in mks units
    '''
    hh = h.value
    k_b = k_B.value
    cl = c.value
    wl = wavelengths*1e-10
    return 2*hh*cl**2 / (wl**5 * (np.exp(hh*cl/(wl*k_b*T)) - 1))

def xyz_to_rgb(X,Y,Z,color_space='sRGB'):
    R =  3.2404542*X - 1.5371385*Y - 0.4985314*Z
    G = -0.9692660*X + 1.8760108*Y + 0.0415560*Z
    B =  0.0556434*X - 0.2040259*Y + 1.0572252*Z
    if color_space == 'sRGB':
        def adj(C):
            mask = abs(C)<0.0031308
            C[mask] *= 12.92
        ...
        raise Exception('Unknown color space {}'.format(color_space))
    return R,G,B


def spectrum_to_rgb(wavelengths,spectral_power_density,color_space='sRGB'):
    '''
    Using the CIE 1931 color matching functions, (corrected by Judd (1951) and Vos (1978)),
    return approximate (R,G,B) values corresponding to a spectrum.

    These should correspond to the human-experienced color under bright lighting --
    dim lighting primarly uses rod cells in the eyes which have different response
    functions.

    For now, supported colorspaces are only RGB and sRGB
    '''
    ciexyz = pd.read_csv(eye_response,header=None,names=["Lambda","x","y","z"])
    ciexyz.Lambda *= 10
    xcmf = interp1d(ciexyz.Lambda,ciexyz.x,kind='quadratic',fill_value=0,bounds_error=False)
    ycmf = interp1d(ciexyz.Lambda,ciexyz.y,kind='quadratic',fill_value=0,bounds_error=False)
    zcmf = interp1d(ciexyz.Lambda,ciexyz.z,kind='quadratic',fill_value=0,bounds_error=False)
    xcmfs = xcmf(wavelengths)
    ycmfs = ycmf(wavelengths)
    zcmfs = zcmf(wavelengths)
    spd = spectral_power_density
    x = simps(xcmfs[:,np.newaxis,np.newaxis]*spd,wavelengths,axis=0)
    y = simps(ycmfs[:,np.newaxis,np.newaxis]*spd,wavelengths,axis=0)
    z = simps(zcmfs[:,np.newaxis,np.newaxis]*spd,wavelengths,axis=0)
    n = x+y+z
    mask = n > 0
    x[mask] /= n[mask]
    y[mask] /= n[mask]
    ...
    r,g,b = xyz_to_rgb(x,y,z,color_space)
    r[np.isnan(r)] = 0
    g[np.isnan(g)] = 0
    b[np.isnan(b)] = 0
    norm = r+g+b
